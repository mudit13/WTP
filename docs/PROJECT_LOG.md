...
## 9. GAN Fingerprints (Yu2019) reproduced in PyTorch
...
## 10. GANFingerprints rebuild: CNN path, train-only PCA, head-to-head benchmark (2026-06-28)

**What:** Rebuilt the GAN-fp attribution method as a DUAL path with a shared, reproducible
head-to-head benchmark:
- **Path B (CNN, new):** `scripts/lib/ganfp_net.py` - a faithful Yu2019 CNN: a FIXED
  (non-trainable, DC-suppressed) high-pass residual front-end (`[[0,1,0],[1,-4,1],[0,1,0]]/4`,
  `requires_grad=False`) feeding 3 VGG-style conv blocks (Conv-BN-ReLU x2 + MaxPool,
  channels [16,32,64]) + AdaptiveAvgPool2d(1) + Linear(64,128)+ReLU+Dropout(0.3)+
  Linear(128,C). ~82K trainable params; single-channel Rec.601 luminance input (256x256),
  no z-scoring (high-pass + BN replaces it). `GANFpDataset` (torch Dataset, lazy torch
...
interpretable but limited by its fixed feature design. A faithful Yu2019 CNN lets the conv
