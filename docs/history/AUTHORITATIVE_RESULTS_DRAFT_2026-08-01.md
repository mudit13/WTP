> **Historical snapshot:** Every number below belongs to the earlier run `2026-08-01_eightway_v1`
> and must not be copied into a report for a later run without re-reading it from that later
> run's own artifacts and release manifest. `CITATIONS.md` (referenced below) no longer exists;
> use `docs/REFERENCES.md`. Kept for wording/structure reference only.
