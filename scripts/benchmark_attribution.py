...
def main(args):
    ...
    if args.sample_dir:
        ...
        X, generator_arr, label_arr, path_arr = ganfp.build_features(
            ...
            bool(augment), qrange, seed, force=args.recompute_features, logger=logger)
        paths_all = [str(p) for p in path_arr]
        generators_all = [str(g) for g in generator_arr]
    else:
    ...
    groups = (io_utils.apply_group_map_with_lookup(paths_kept_arr, lookup_map, group_map, logger=logger)
