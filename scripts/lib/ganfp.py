...
def extract_fingerprints(image_paths: Sequence[str],
    ...
    and excluded from kept_paths so the caller keeps labels aligned; each skip is warned via
    `logger` (if given) so drops are never silent. The caller can also derive per-generator
    skip counts by diffing its labels against kept_paths. `batch_size` is accepted for
    signature parity with clip_features.extract_features (no GPU batching here).
    """
    from . import image_ops
    ...
    for path in image_paths:
        ...
        except Exception as exc:  # noqa: BLE001 - skip unreadable files, keep the run going
...
def _report_skips(all_paths: Sequence[str], all_generators: Sequence[str],
                  kept: Sequence[str], logger=None) -> dict:
    """Per-generator count of images that failed to load (were dropped from the feature matrix).
    Returned as a dict AND logged, so an unreadable-file bias toward one generator/class is
    visible instead of silently shrinking that class."""
    kept_set = set(kept)
    counts = {}
    for p, g in zip(all_paths, all_generators):
        if p not in kept_set:
            counts[str(g)] = counts.get(str(g), 0) + 1
    total = sum(counts.values())
    if logger is not None:
        if total:
            logger.warning("Skipped %d unreadable images; per-generator: %s", total, counts)
        else:
            logger.info("No images skipped (all %d readable).", len(all_paths))
    return counts
...
def features_from_samples(paths: Sequence[str], generators: Sequence[str],
    ...
    Returns (X, generator, label, paths) as aligned arrays. Per-generator skip counts for
    unreadable images are logged via `logger` (if given).
    """
    X, kept = extract_fingerprints(paths, common_size, feat_size, mode, augment, logger=logger)
    _report_skips(paths, generators, kept, logger)
...
def build_features(index_csv: str, cache_path: Optional[str], common_size: int = 256,
    ...
    X, kept = extract_fingerprints(paths, common_size, feat_size, mode, augment, logger=logger)
    _report_skips(paths, df[schema.GENERATOR].astype(str).tolist(), kept, logger)
...
def extract_dct_features(image_paths: Sequence[str], common_size: int = 256,
                         block: int = 8, augment=None, logger=None
                         ) -> Tuple[np.ndarray, List[str]]:
    ...
    time JPEG augmentation, same augmenter object the residual/spectrum path uses). Unreadable
    images are skipped and warned via `logger` (if given), never dropped silently."""
    from . import image_ops
